...
```shell-session
frank-106@htb[/htb]$ git clone https://github.com/iagox86/dnscat2.git
...
```shell-session
frank-106@htb[/htb]$ sudo ruby dnscat2.rb --dns host=10.10.14.18,port=53,domain=inlanefreight.local --no-cache
...
```shell-session
frank-106@htb[/htb]$ git clone https://github.com/lukebaggett/dnscat2-powershell.git
```
