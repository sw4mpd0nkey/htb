...
```shell-session
frank-106@htb[/htb]$ git clone https://github.com/utoni/ptunnel-ng.git
```
...
```shell-session
frank-106@htb[/htb]$ sudo ./autogen.sh 
```
...
```shell-session
frank-106@htb[/htb]$ sudo apt install automake autoconf -y
frank-106@htb[/htb]$ cd ptunnel-ng/
frank-106@htb[/htb]$ sed -i '$s/.*/LDFLAGS=-static "${NEW_WD}\/configure" --enable-static $@ \&\& make clean \&\& make -j${BUILDJOBS:-4} all/' autogen.sh
frank-106@htb[/htb]$ ./autogen.sh
```
...
```shell-session
frank-106@htb[/htb]$ scp -r ptunnel-ng ubuntu@10.129.202.64:~/
```
...
```shell-session
frank-106@htb[/htb]$ sudo ./ptunnel-ng -p10.129.202.64 -l2222 -r10.129.202.64 -R22
...
```shell-session
frank-106@htb[/htb]$ ssh -p2222 -lubuntu 127.0.0.1
...
```shell-session
frank-106@htb[/htb]$ ssh -D 9050 -p2222 -lubuntu 127.0.0.1
...
```shell-session
frank-106@htb[/htb]$ proxychains nmap -sV -sT 172.16.5.19 -p3389
