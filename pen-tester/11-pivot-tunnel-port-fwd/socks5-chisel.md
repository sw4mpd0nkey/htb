...
```shell-session
frank-106@htb[/htb]$ git clone https://github.com/jpillora/chisel.git
```
...
```shell-session
frank-106@htb[/htb]$ scp chisel ubuntu@10.129.202.64:~/
...
```shell-session
frank-106@htb[/htb]$ ./chisel client -v 10.129.202.64:1234 socks
...
```shell-session
frank-106@htb[/htb]$ tail -f /etc/proxychains.conf 
...
```shell-session
frank-106@htb[/htb]$ sudo ./chisel server --reverse -v -p 1234 --socks5
