...
```shell-session
swmpdnky@htb[/htb]$ gem install evil-winrm
```
...
```shell-session
swmpdnky@htb[/htb]$ evil-winrm -i 10.129.201.234 -u forend
...
```shell-session
swmpdnky@htb[/htb]$ mssqlclient.py 
...
```shell-session
swmpdnky@htb[/htb]$ mssqlclient.py INLANEFREIGHT/DAMUNDSEN@172.16.5.150 -windows-auth
