...
```shell-session
frank-106@htb[/htb]$ sudo tcpdump -i ens224 
```

![image](imgs/tcpdump-example.gif)
...
![image](imgs/responder-example.gif)
...
```shell-session
frank-106@htb[/htb]$ fping -asgq 172.16.5.0/23
...
```shell-session
frank-106@htb[/htb]$ nmap -A 172.16.5.100
...
```shell-session
frank-106@htb[/htb]$ sudo git clone https://github.com/ropnop/kerbrute.git
...
```shell-session
frank-106@htb[/htb]$ sudo make all
...
```shell-session
frank-106@htb[/htb]$ ./kerbrute_linux_amd64 
...
```shell-session
frank-106@htb[/htb]$ sudo mv kerbrute_linux_amd64 /usr/local/bin/kerbrute
```
...
```shell-session
frank-106@htb[/htb]$ kerbrute userenum -d INLANEFREIGHT.LOCAL --dc 172.16.5.5 jsmith.txt -o valid_ad_users
