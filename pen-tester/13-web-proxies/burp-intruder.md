...
We can then go to `Intruder` by clicking on its tab or with the shortcut \[`CTRL+SHIFT+I`\], which takes us right to `Burp Intruder` On the first tab, '`Target`', we see the details of the target we will be fuzzing, which is fed from the request we sent to `Intruder`.
