...
```shell-session
frank-106@htb[/htb]$ proxychains curl http://SERVER_IP:PORT
...
```shell-session
frank-106@htb[/htb]$ nmap -h | grep -i prox
...
```shell-session
frank-106@htb[/htb]$ nmap --proxies http://127.0.0.1:8080 SERVER_IP -pPORT -Pn -sC
