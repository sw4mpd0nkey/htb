...
`Burp`: ![Burp request details](imgs/burp_history_details.jpg)

`ZAP`: ![ZAP request details](imgs/zap_history_details.jpg)
...
![Burp repeat request](imgs/burp_repeater_request.jpg)
...
In ZAP, once we locate our request, we can right-click on it and select `Open/Resend with Request Editor`, which would open the request editor window, and allow us to resend the request with the `Send` button to send our request: ![ZAP resend request](imgs/zap_repeater_request.jpg)
...
![ZAP HUD resend](imgs/ap_hud_resend.jpg)
...
![Burp modify repeat](imgs/burp_repeat_modify.jpg)
