...
```shell-session
swmpdnky@htb[/htb]$ echo '<!ENTITY joined "%begin;%file;%end;">' > xxe.dtd
swmpdnky@htb[/htb]$ python3 -m http.server 8000
...
Once we write our `xxe.dtd` file, host it on our machine, and then add the above lines to our HTTP request to the vulnerable web application, we can finally get the content of the `submitDetails.php` file: ![php_cdata](imgs/web_attacks_xxe_php_cdata.jpg)
...
Once we host our DTD script as we did earlier and send the above payload as our XML data (no need to include any other XML data), we will get the content of the `/etc/hosts` file as follows: ![exfil_error](imgs/web_attacks_xxe_exfil_error_2.jpg)
