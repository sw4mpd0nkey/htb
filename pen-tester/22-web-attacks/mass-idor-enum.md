...
![](imgs/web_attacks_idor_documents.jpg)
...
```shell-session
swmpdnky@htb[/htb]$ curl -s "http://SERVER_IP:PORT/documents.php?uid=1" | grep "<li class='pure-tree_link'>"
...
```shell-session
swmpdnky@htb[/htb]$ curl -s "http://SERVER_IP:PORT/documents.php?uid=3" | grep -oP "\/documents.*?.pdf"
