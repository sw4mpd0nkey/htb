...
```shell-session
swmpdnky@htb[/htb]$ nmap -p 80,443,8000,8080,8180,8888,10000 --open -oA web_discovery -iL scope_list
```
...
```shell-session
swmpdnky@htb[/htb]$ cat scope_list 
...
```shell-session
swmpdnky@htb[/htb]$ sudo  nmap -p 80,443,8000,8080,8180,8888,10000 --open -oA web_discovery -iL scope_list 
...
```shell-session
swmpdnky@htb[/htb]$ sudo nmap --open -sV 10.129.201.50
...
```shell-session
swmpdnky@htb[/htb]$ sudo apt install eyewitness
```
...
```shell-session
swmpdnky@htb[/htb]$ eyewitness --web -x web_discovery.xml -d inlanefreight_eyewitness
...
```shell-session
swmpdnky@htb[/htb]$ wget https://github.com/michenriksen/aquatone/releases/download/v1.7.0/aquatone_linux_amd64_1.7.0.zip
```

Application Discovery & Enumeration

```shell-session
swmpdnky@htb[/htb]$ unzip aquatone_linux_amd64_1.7.0.zip 
...
```shell-session
swmpdnky@htb[/htb]$ cat web_discovery.xml | ./aquatone -nmap
