...
```shell-session
swmpdnky@htb[/htb]$ curl -s http://blog.inlanefreight.local | grep WordPress
...
```shell-session
swmpdnky@htb[/htb]$ curl -s http://blog.inlanefreight.local/ | grep themes
...
```shell-session
swmpdnky@htb[/htb]$ curl -s http://blog.inlanefreight.local/ | grep plugins
...
```shell-session
swmpdnky@htb[/htb]$ curl -s http://blog.inlanefreight.local/?p=1 | grep plugins
...
```shell-session
swmpdnky@htb[/htb]$ sudo gem install wpscan
```
...
```shell-session
swmpdnky@htb[/htb]$ sudo wpscan --url http://blog.inlanefreight.local --enumerate --api-token dEOFB<SNIP>
