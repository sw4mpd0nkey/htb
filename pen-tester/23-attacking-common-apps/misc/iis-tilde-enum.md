...
```shell-session
swmpdnky@htb[/htb]$ nmap -p- -sV -sC --open 10.129.224.91
...
```shell-session
swmpdnky@htb[/htb]$ java -jar iis_shortname_scanner.jar 0 5 http://10.129.204.231/
...
```shell-session
swmpdnky@htb[/htb]$ egrep -r ^transf /usr/share/wordlists/* | sed 's/^[^:]*://' > /tmp/list.txt
```
...
```shell-session
swmpdnky@htb[/htb]$ gobuster dir -u http://10.129.204.231/ -w /tmp/list.txt -x .aspx,.asp
