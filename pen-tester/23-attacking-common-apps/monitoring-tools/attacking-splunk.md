...
```shell-session
swmpdnky@htb[/htb]$ tree splunk_shell/
...
```shell-session
swmpdnky@htb[/htb]$ cat inputs.conf 
...
```shell-session
swmpdnky@htb[/htb]$ tar -cvzf updater.tar.gz splunk_shell/
...
```shell-session
swmpdnky@htb[/htb]$ sudo nc -lnvp 443
