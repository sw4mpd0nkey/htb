...
```shell-session
swmpdnky@htb[/htb]$ sudo nmap -sV -p- --open -T4 10.129.201.50
...
```shell-session
swmpdnky@htb[/htb]$ curl -s http://10.129.201.50:8080/index.htm -A "Mozilla/5.0 (compatible;  MSIE 7.01; Windows NT 5.0)" | grep version
...
```shell-session
swmpdnky@htb[/htb]$ sudo crackmapexec smb 10.129.201.50 -u prtgadm1 -p Pwn3d_by_PRTG! 
