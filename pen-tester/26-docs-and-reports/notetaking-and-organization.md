...
```shell-session
swmpdnky@htb[/htb]$ git clone https://github.com/tmux-plugins/tpm ~/.tmux/plugins/tpm
```
...
```shell-session
swmpdnky@htb[/htb]$ touch .tmux.conf
```
...
```shell-session
swmpdnky@htb[/htb]$ cat .tmux.conf 
...
```shell-session
swmpdnky@htb[/htb]$ tmux source ~/.tmux.conf 
```
...
![image](imgs/tmux_log_enable.gif)
...
![image](imgs/tmux_pane_capture.gif)
...
```shell-session
swmpdnky@htb[/htb]$ mkdir -p ACME-IPT/{Admin,Deliverables,Evidence/{Findings,Scans/{Vuln,Service,Web,'AD Enumeration'},Notes,OSINT,Wireless,'Logging output','Misc Files'},Retest}
```

Notetaking & Organization

```shell-session
swmpdnky@htb[/htb]$ tree ACME-IPT/
...
![image](imgs/terminal_output.png)
...
```shell-session
swmpdnky@htb[/htb]$ xfreerdp /v:10.129.203.82 /u:htb-student /p:HTB_@cademy_stdnt!
```
